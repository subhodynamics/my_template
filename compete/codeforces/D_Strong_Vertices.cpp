...
void solve() {
    ...
    vector<pair<ll, ll>> allVertices;
    ll maxx = -1e18;
    for (int i = 0; i < n; i++) {
        allVertices.pb({a[i] - b[i], i+1});
        maxx = max(maxx,a[i]-b[i]);
    }
    vector<ll> strong;
    for (int i = 0; i < n; i++) {
        if (maxx == allVertices[i].ff) strong.pb(allVertices[i].ss);
    }
    cout << sz(strong) << nline;
    for (auto x : strong) cout << x << " ";
    cout << nline;
}
