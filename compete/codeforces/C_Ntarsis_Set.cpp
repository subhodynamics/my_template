...
void solve() {
    ...
    while (lo <= hi) {
        ...
        for (int i = 0; i < k; i++) {
            ll idx = (upper_bound(all(arr), find) - arr.begin());
            // find -= idx; //idx is the index for a num > find, find will have the diff of idx
            // if idx == find; or find - idx = 0, means what i am trying to find is on the left
            find -= idx; 
        }
