...
// to check whethre two a node is ansistor of the other node or not
// inX < inY < outY < outX
...
bool isAnsistor(int x, int y) {
    return ((inTime[x] < inTime[y]) && (outTime[y] < outTime[x]));
}

void dfs (int current, vector<vector<int>> &adj, int parent) {
    inTime[current] = globalTime++;
    for (auto neighbour : adj[current]) {
        if (neighbour != parent) {
            dfs (neighbour, adj, current);
        }
    }
