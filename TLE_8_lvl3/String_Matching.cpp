...
void solve() {
    string str, patt; cin >> ws >> str >> ws >> patt;

    int n = sz(str), m = sz(patt);

    Hashing str_hash (str); // hashValue of the string is now available => like prefix array
    Hashing patt_hash (patt); // hashValue of the patter is now available => like prefix array

    vector<ll> pooraPatternHash = patt_hash.substringHash(0, m - 1);

    int ans = 0;
    
    for (int i = 0; (i + m - 1) < n; i++) {
        ll l = i, r = i + m - 1;
        vector<ll> substring_strHash = str_hash.substringHash(l, r);
        debug(substring_strHash)
        ans += (substring_strHash == pooraPatternHash);
    }
    cout << ans << nline;
}
