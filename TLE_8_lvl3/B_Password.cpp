...
bool check(int l, Hashing hash_s, int n) {
    for (int i = 1; i+l < n-1; i++) {
        if (hash_s.compareSubstrings(i, i+l, 0, l)) return true;
    }
    return false;
}
...
void solve() {
    ...
    for (int i = 0; i < n-1; i++) {
        // vector<ll> prefix = hash_s.substringHash(0, i);
        // vector<ll> suffix = hash_s.substringHash((n-1)-i, n-1);
        if (hash_s.compareSubstrings(0, i, (n-1)-i, n-1)) same.pb(i); //hash of this length is same for suffix and prefix
    }

    int lo = 0, hi = sz(same)-1; 
    ...
    if (found) cout << s.substr(0, ans+1) << nline;
    else cout << "Just a legend" << nline;
